...
% Oct 08, 2012: process multi-parameters of each method
...
% test cases
    ...
    fileN(27).name = 'nf100_len1.835288e+001_ps1.00e-007_noise.tif';
...
iNF  = 8%23:24%[7 21:24];  %number of the images to be processed
...
PO.CT = [1 5 10 20 30];% CT threshold percentage, parameters to be optimised,
PO.GF =  ['0.25';'0.50'; '1.00'; '2.00'; '3.00']; % parameters to be optimised,
PO.STV = [4 8 12 16 20]; % parameters to be optimised,
PO.TUB = ['0.5';'1.5';'2.5';'3.5';'4.5']; % parameters to be optimised,
% set the method to run, 1: run; 0:not run
runCT = 0; runCA =0; runGF = 1; runSTV = 0; runTUB = 0; % 
...
%  pause
    ...
    for iP = 1:5
%         fmat1 = [dir6,sprintf('test%d-FIREout.mat',iN)];
        fmat2 = [dir6,sprintf('test%d-FIREoutCTr_P%d.mat',iN,iP)];
        fmat3 = [dir6,sprintf('test%d-CAout_P%d.mat',iN,iP)];
        fmat4 = [dir6,sprintf('test%d-FIREoutGF_P%d.mat',iN,iP)];
        fmat5 = [dir6,sprintf('test%d-FIREoutSTV_P%d.mat',iN,iP)];
        fmat6 = [dir6,sprintf('test%d-FIREoutTUB_P%d.mat',iN,iP)];

        ffigO = [dir6,sprintf('test%d-FIREfigO.tif',iN)]; % original figure
        ffigE = [dir6,sprintf('test%d-FIREfigE_P%d.tif',iN,iP)]; % extracted figure
%         fctr = [dir5, sprintf('test%d-CTR_P%d.mat',iN,iP)];  % filename of the curvelet transformed reconstructed image
%         fgf = [dir5, sprintf('test%d_GFf_P%d.mat',iN,iP)];  % filename of the Gaussian filtered image
       ...
       fgf = [dir5,'\gau_mult_params\',sprintf('test%d_GFf_%s.mat',iN,PO.GF(iP,:))];  % filename of the Gaussian filtered image

    
        %         fstv = [dir5, '\sptvx_mult_param\',sprintf('%s8.mat_%d.TIF',fileN(iN).name),iP];  % filename of the spirial TV filtered image
        if iN < 10
             fstv = [dir5, '\sptvx_mult_param\',sprintf('test0%d.tif_%d.tif',iN,PO.STV(iP))];  % filename of the spirial TV filtered image
        else
             if iN == 23 | iN == 24
                fstv = [dir5, '\sptvx_mult_param\',sprintf('test%d.tiff_%d.tif',iN,PO.STV(iP))];  % filename of the spirial TV filtered image
            else
                fstv = [dir5, '\sptvx_mult_param\',sprintf('test%d.tif_%d.tif',iN,PO.STV(iP))];  % filename of the spirial TV filtered image
        ...
        if iN < 10
             ftub = [dir5,'\tubeness_mult_params\',sprintf('test0%d.tif_tube_%s.tif',iN,PO.TUB(iP,:))];  % filename of the tube filtered image
        else
            if iN == 23 | iN == 24
                ftub = [dir5,'\tubeness_mult_params\',sprintf('test%d.tiff_tube_%s.tif',iN,PO.TUB(iP,:))];  % filename of the tube filtered image
            else
                ftub = [dir5,'\tubeness_mult_params\',sprintf('test%d.tif_tube_%s.tif',iN,PO.TUB(iP,:))];  % filename of the tube filtered image
            end   
        end
...
%       fOL1 = [dir4, sprintf('test%d_OL_FIRE.tif',iN)]; %filename of overlay 1
        fOL2 = [dir4, sprintf('test%d_OL_CTpFIRE_P%d.tif',iN,iP)]; %filename of overlay 2
        fOL3 = [dir4, sprintf('test%d_OL_CA.tif',iN)]; %filename of overlay 3
        fOL4 = [dir4, sprintf('test%d_OL_GFFIRE_P%d.tif',iN,iP)]; %filename of overlay 4
        fOL5 = [dir4, sprintf('test%d_OL_STVFIRE_P%d.tif',iN,iP)]; %filename of overlay 5
        fOL6 = [dir4, sprintf('test%d_OL_TUBFIRE_P%d.tif',iN,iP)]; %filename of overlay 6
...
%run main FIRE code 
    ...
    if iN == 0% 
        ...
        home
        disp(sprintf('Reconstructed image of Test%d, P%d has been processed',iN,iP))
        data = data2; 
...
%run main FIRE code 
    ...
    if iN == 0% 
        ...
        home
        disp(sprintf('Gaussian filtered image of Test%d, P%d has been processed',iN,iP))
        data = data4; 
...
%% run FIRE on spirial tv fiterered image
if runSTV == 1
...
%run main FIRE code 
    ...
    if iN == 0% 
        ...
        home
        disp(sprintf('STV filtered image of Test%d, p%d has been processed',iN,iP))
        data = data5; 
        save(fmat5,'data','fname','iN','p','iP');
    end 
...
%% run FIRE on tube fiterered image
if runTUB == 1
    
    disp(sprintf('running FIRE on tubeness fiterered image, test#%d, para#%d',iN,iP)); 
...
%run main FIRE code 
    ...
    if iN == 0% 
        ...
        home
        disp(sprintf('Tube filtered image of Test%d, P%d, has been processed',iN,iP))
        data = data6; 
        save(fmat6,'data','fname','iN','p','iP');
    end 
...
if runCA == 1 & runCT == 1 & runGF == 1 & runSTV == 1 & runTUB ==1
    figure(53);clf;
